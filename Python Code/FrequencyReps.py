...
wandb.init(project = "ClassificationWithFrequencies")
    ...
    for root, dirs, files in os.walk(data_dir):
        for file in files:
            if file.endswith('.mat') and 'EEG' in file:
                ...
                for item in range(3):
                    ...
                    EEGdft = np.fft.fft(EEGList[item])
                    fs = 256  # Example sampling frequency (adjust as needed)
                    n = len(EEGList[item])
                    EEGdft = EEGdft[0:int(n/2)+1]
                    PSDEEG = (1/(fs*n)) * abs(EEGdft)**2
                    PSDEEG[1:-2] = 2 * PSDEEG[1:-2]
                    freq_EEG = np.arange(0, fs/2 + fs/n, fs/n)
                    PSDEEG = PSDEEG[0:int(np.floor((len(CleanedC3)*2.5)/256))] # This period is set to consider the frequencies from 0-2.5Hz
                    PSDList.append(PSDEEG)
        ...
        if len(files) != 0 and any('.mat' in word for word in files):
            ...
            if len(PSDList[0]) == len(PSDECG):
                ...
                ConcatenateData = np.concatenate((PSDList[0], PSDList[1], PSDList[2], PSDECG[0:len(PSDList[0])]), axis=1, dtype = np.double)
            
            ConcatenateData = ConcatenateData[:, 0:4] # Use only PSDECG to check the results and add other channels later on!!!!
...
hidden_size2 = 16  # Number of hidden units in the LSTM layer
hidden_size3 = 8
num_layer = 1  # Number of LSTM layers
num_classes = 2  # Number of output classes
D = 1
