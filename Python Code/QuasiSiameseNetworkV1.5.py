'''
This is the version that I did upsampling for SWI and coherence to match the size of HR. The main difference compared to the QuasiSiameseNetwork.py file.
'''
import os
...
def DataPreparation2(data_dir):
    ...
    for root, dirs, files in os.walk(data_dir):
            ...
            if file.endswith('.mat') and 'EEG' in file:
                ...
                for item in range(3):
                    ...
                    for iter in range(int(len(CleanedC3)/(256*300))):
                        ...
                        Beta_band_indices = np.where((frequencies >= 13) & (frequencies <= 30))[0]       # We calculate the power for Theta frequency band!
                        ...
                        power_in_Beta_band = np.sum(PSDEEG[Beta_band_indices])
                        # SWI format
                        DSI = power_in_Delta_band / (power_in_Theta_band + power_in_Alpha_band + power_in_Beta_band)
                        TSI = power_in_Theta_band / (power_in_Delta_band + power_in_Alpha_band + power_in_Beta_band)
                        ASI = power_in_Alpha_band / (power_in_Delta_band + power_in_Theta_band + power_in_Beta_band)
                        BSI = power_in_Beta_band / (power_in_Delta_band + power_in_Theta_band + power_in_Alpha_band)
                        
                        # Relative powers format
                        '''DTP = power_in_Delta_band / (power_in_Delta_band + power_in_Theta_band + power_in_Alpha_band + power_in_Beta_band)
                        ThTP = power_in_Theta_band / (power_in_Delta_band + power_in_Theta_band + power_in_Alpha_band + power_in_Beta_band)
                        ATP = power_in_Alpha_band / (power_in_Delta_band + power_in_Theta_band + power_in_Alpha_band + power_in_Beta_band)
                        BTP = power_in_Beta_band / (power_in_Delta_band + power_in_Theta_band + power_in_Alpha_band + power_in_Beta_band)'''
                        # SWI format
                        DSIList.append(DSI)
                        TSIList.append(TSI)
                        ASIList.append(ASI)
                    ...
                    PSDList.append(BSIList)
                    # Relative powers format
                    '''PSDList.append(DTPList)
                    PSDList.append(ThTPList)
                    PSDList.append(ATPList)
                    PSDList.append(BTPList)'''
        ...
        if len(files) != 0 and any('.mat' in word for word in files):
            ...
            # Change it when you want to have three or four EEG frequency band!
            ConcatenateData = np.concatenate((
                ...
                np.array(PSDList[9])[:, np.newaxis], np.array(PSDList[10])[:, np.newaxis], np.array(PSDList[11])[:, np.newaxis]
            ), axis=1, dtype=np.double)

            ####################### USE F3 channel to check the results of one channel first! ################################
            ConcatenateData = ConcatenateData[:, 0:12]
...
def DataPreparation3(data_dir):
        ...
        for file in files:
            if file.endswith('.mat') and 'C3' in root:
                ...
                for item in range(len(mat['MagCoherence'][0])):
                    MagCoherenceC3 = mat['MagCoherence'][128:256, item] ### This value is based on the frequencies that we want to capture in our analysis.(up to 10Hz with sampling rate of 32)
                ...
                mat = loadmat(mat_file)
                for item in range(len(mat['MagCoherence'][0])):
                    MagCoherenceF3 = mat['MagCoherence'][128:256, item] ### 4-8 Hz frequencies (128-256) since Theta band showed the significant difference between the cases.
                ...
                mat = loadmat(mat_file)
                for item in range(len(mat['MagCoherence'][0])):
                    MagCoherenceO1 = mat['MagCoherence'][128:256, item]
    ...
    if len(files) != 0 and any('.mat' in word for word in files):
        ...
        newcoh = ConcatenateCoher.reshape(len(O1_labels), len(MagCoherenceO1), 3)
...
def DataPreparation(BPs, EEGLabels, HRs, Labels, Coherences, C_labels):
    ...
    for item in range(len(Labels)):
        ...
        #downsample_factor = len(HRs[item]) // len(BPs[item])  # Compute downsampling factor
        #downsampled_signal_m = HRs[item][::downsample_factor]
        signal_m = np.reshape(HRs[item], (len(HRs[item]), 1))
        newHRs.append(signal_m)
        if len(HRs[item]) == len(BPs[item]) == len(Coherences[item]):
            newHRs[item] = newHRs[item].reshape(-1, 1)
            #ConcatenateData = np.concatenate((BPs[item], newHRs[item]), axis=1, dtype = np.double)

        elif (len(HRs[item]) > len(BPs[item])) and (len(HRs[item]) > len(Coherences[item])):
            zoom_factor1 = (len(HRs[item]) / len(BPs[item]), 1)
            zoom_factor2 = (len(HRs[item]) / len(Coherences[item]), 1)
            BPs[item] = zoom(BPs[item], zoom_factor1, order = 1)
            Coherences[item] = zoom(Coherences[item], zoom_factor2, order = 1)
            newHRs[item] = newHRs[item].reshape(-1, 1)
...
class QuasiSiamese(nn.Module):
    def __init__(self, input1_size, input2_size, input3_size, hidden_size1, num_lstm_layer, num_classes, D, kernel_size, dropout):
        ...
        # 1DCNN layer
        self.conv11 = nn.Conv1d(in_channels=input1_size, out_channels=16, kernel_size=self.kernel_size)
        self.batchnorm11 = nn.BatchNorm1d(16)
        self.relu11 = nn.ReLU()
        self.pool11 = nn.MaxPool1d(kernel_size=3)
        self.conv12 = nn.Conv1d(in_channels=16, out_channels=32, kernel_size=self.kernel_size)
        self.batchnorm12 = nn.BatchNorm1d(32)
        self.relu12 = nn.ReLU()
        self.pool12 = nn.MaxPool1d(kernel_size=3)
        self.conv13 = nn.Conv1d(in_channels=32, out_channels=64, kernel_size=self.kernel_size)
        self.batchnorm13 = nn.BatchNorm1d(64)
        self.relu13 = nn.ReLU()
        self.pool13 = nn.MaxPool1d(kernel_size=3)       
        # LSTM layer
        self.lstm11 = nn.LSTM(input_size=32, hidden_size=self.hidden_size1, batch_first=True, num_layers= self.num_lstm_layer, dropout=self.dropout, bidirectional=True)
        
        ###### Second CNN-LSTM for SWIs classification
        # 1DCNN layer
        self.conv21 = nn.Conv1d(in_channels=input2_size, out_channels=16, kernel_size=self.kernel_size)
        self.batchnorm21 = nn.BatchNorm1d(16)
        self.relu21 = nn.ReLU()
        self.pool21 = nn.MaxPool1d(kernel_size=3)
        self.conv22 = nn.Conv1d(in_channels=16, out_channels=32, kernel_size=self.kernel_size)
        self.batchnorm22 = nn.BatchNorm1d(32)
        self.relu22 = nn.ReLU()
        self.pool22 = nn.MaxPool1d(kernel_size=3)
        self.conv23 = nn.Conv1d(in_channels=32, out_channels=64, kernel_size=self.kernel_size)
        self.batchnorm23 = nn.BatchNorm1d(64)
        self.relu23 = nn.ReLU()
        self.pool23 = nn.MaxPool1d(kernel_size=3)       
        # LSTM layer
        self.lstm21 = nn.LSTM(input_size=32, hidden_size=self.hidden_size1, batch_first=True, num_layers= self.num_lstm_layer, dropout=self.dropout, bidirectional=True)
        
        ###### Third CNN-LSTM for Coherences classification
        # 1DCNN layer
        self.conv31 = nn.Conv1d(in_channels=input3_size, out_channels=16, kernel_size=self.kernel_size)
        self.batchnorm31 = nn.BatchNorm1d(16)
        self.relu31 = nn.ReLU()
        self.pool31 = nn.MaxPool1d(kernel_size=3)
        self.conv32 = nn.Conv1d(in_channels=16, out_channels=32, kernel_size=self.kernel_size)
        self.batchnorm32 = nn.BatchNorm1d(32)
        self.relu32 = nn.ReLU()
        self.pool32 = nn.MaxPool1d(kernel_size=3)
        self.conv33 = nn.Conv1d(in_channels=32, out_channels=64, kernel_size=self.kernel_size)
        self.batchnorm33 = nn.BatchNorm1d(64)
        self.relu33 = nn.ReLU()
        self.pool33 = nn.MaxPool1d(kernel_size=3)       
        # LSTM layer
        self.lstm31 = nn.LSTM(input_size=32, hidden_size=self.hidden_size1, batch_first=True, num_layers= self.num_lstm_layer, dropout=self.dropout, bidirectional=True)

        ####### Fully connected layer
        self.fc = nn.Sequential(
            nn.Linear(3*self.D*self.hidden_size1, 16), ##### change the first number based on the number of input values.
            nn.ReLU(),
    ...
    def forward(self, x1, x2, x3):
        ...
        x1 = x1.permute(0, 2, 1)
        h0 = torch.randn(self.D*self.num_lstm_layer, x1.size(0), self.hidden_size1).to(x1.device)
        h0 = torch.tensor(h0, dtype=torch.double)
        c0 = torch.randn(self.D*self.num_lstm_layer, x1.size(0), self.hidden_size1).to(x1.device)
        c0 = torch.tensor(c0, dtype=torch.double)
        ...
        x2 = x2.permute(0, 2, 1)
        h0 = torch.randn(self.D*self.num_lstm_layer, x2.size(0), self.hidden_size1).to(x2.device)
        h0 = torch.tensor(h0, dtype=torch.double)
        c0 = torch.randn(self.D*self.num_lstm_layer, x2.size(0), self.hidden_size1).to(x2.device)
        c0 = torch.tensor(c0, dtype=torch.double)
        ...
        x3 = x3.permute(0, 2, 1)
        h0 = torch.randn(self.D*self.num_lstm_layer, x3.size(0), self.hidden_size1).to(x3.device)
        h0 = torch.tensor(h0, dtype=torch.double)
        c0 = torch.randn(self.D*self.num_lstm_layer, x3.size(0), self.hidden_size1).to(x3.device)
        c0 = torch.tensor(c0, dtype=torch.double)
...
def HyperparameterTuning(lr, batch_size, num_lstm_layer, kernel_size, dropout, trainval_index, trainval_subset_loader):
    ...
    for train_index, val_index in kf.split(np.zeros(len(intargets)), intargets):
        ...
        input_size1 = 1
        input_size2 = 12 # Change it when changing the number of features of Powers
        input_size3 = 3
        ...
        model = QuasiSiamese(input_size1, input_size2, input_size3, hidden_size1, num_lstm_layer, num_classes, D, kernel_size, dropout).to(device)
        model = model.double()
        ...
        wandb.log({"lr": lr,  "batch_size": batch_size, "Kernel_size": kernel_size, "Dropout":dropout, "val_accuracy": val_accuracy, "val_loss": val_loss/len(val_subset_loader), "Num layers": num_layer})
...
for trainval_index, test_index in kfOuter.split(np.zeros(len(targets)), targets):
    ...
    learning_rates = [0.001] #[0.001, 0.01] #[0.001]
    batch_sizes = [128] #[64, 128, 256, 512] #[128]
    Num_layers = [2] #[2, 3, 4, 5] #[2]
    CNN_Kernel_size = [3] #[3, 5] #[3]
    Dropout = [0.1] #[0, 0.1, 0.2, 0.3] #[0.1]
    early_stopping_counter = 0
    for lr, batch_size, num_layer, Kernel_size, dropout in product(learning_rates, batch_sizes, Num_layers, CNN_Kernel_size, Dropout):
        mean_accuracy = HyperparameterTuning(lr, batch_size, num_layer, Kernel_size, dropout, trainval_index, trainval_subset_loader)
        MeanAccuracies.append(mean_accuracy)
        if mean_accuracy > best_accuracy:
            best_accuracy = mean_accuracy
            best_hyperparams = {"lr": lr, "batch_size": batch_size, "Num_layer": num_layer, "CNN_Kernel_size": Kernel_size, "Dropout": dropout}
    ...
    num_lstm_layer = best_hyperparams["Num_layer"]
    kernel_size = best_hyperparams["CNN_Kernel_size"]
    ...
    model2 = QuasiSiamese(input_size1, input_size2, input_size3, hidden_size1, num_lstm_layer, num_classes, D, kernel_size, dropout).to(device)
