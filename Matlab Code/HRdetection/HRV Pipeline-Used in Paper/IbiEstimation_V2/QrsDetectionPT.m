function qrsDetection = QrsDetectionPT(fs, qrsPreProcesing)
%QRSDETECTIONPT
% 
%
...
PEAK_DETECTION.NB_T_WAVE_LOCATION_SAMPLES = round(PEAK_DETECTION.T_WAVE_LOCATION*fs);
PEAK_DETECTION.NB_PEAK_SEARCH_SAMPLES = round(PEAK_DETECTION.PEAK_SEARCH_DURATION*fs);
PEAK_DETECTION.NB_SLOPE_SEARCH_SAMPLES = round(PEAK_DETECTION.SLOPE_SEARCH_DURATION*fs);
...
% 200 msec distance.
PEAK_DETECTION.NB_REFACTORY_SAMPLES = round(PEAK_DETECTION.REFACTORY_PERIOD*fs);
try
    [pks,locs] = findpeaks(ecg_int,'MINPEAKDISTANCE',PEAK_DETECTION.NB_REFACTORY_SAMPLES);
catch
    pks = [];
    locs = [];
end    
...
PEAK_DETECTION.NB_LEARNING1_SAMPLES = min(PEAK_DETECTION.LEARNING1_DURATION*fs,length(ecg_int));

%********** Initialize the integrated signal thresholds (THR_SIG and THR_NOISE) **********% 
THI1 = max(ecg_int(1:PEAK_DETECTION.NB_LEARNING1_SAMPLES))*1/3; % 0.333 of the max amplitude 
THI2 = mean(ecg_int(1:PEAK_DETECTION.NB_LEARNING1_SAMPLES))*1/2;% 0.5 of the mean signal is considered to be noise
...
THF1 = max(ecg_n(1:PEAK_DETECTION.NB_LEARNING1_SAMPLES))*1/3; % 0.333 of the max amplitude 
THF2 = mean(ecg_n(1:PEAK_DETECTION.NB_LEARNING1_SAMPLES))*1/2; % 0.5 of the mean signal is considered to be noise
...
for i = 1 : nbPeaks  
    ...
    if locs(i)-PEAK_DETECTION.NB_PEAK_SEARCH_SAMPLES >= 1 && locs(i) <= length(ecg_n)
          [y_i,x_i] = max(ecg_n(locs(i)-PEAK_DETECTION.NB_PEAK_SEARCH_SAMPLES:locs(i)));
    else
        if i == 1
            [y_i,x_i] = max(ecg_n(1:locs(i)));
            searh_back = 1;
        elseif locs(i)>= length(ecg_n)
            [y_i,x_i] = max(ecg_n(locs(i)-PEAK_DETECTION.NB_PEAK_SEARCH_SAMPLES:end));
        end       
    end       
    ...
    if test_m
        if (locs(i) - qrs_i(Beat_C)) >= round(1.66*test_m)  % Missed QRS 
            ...
            startIdx = qrs_i(Beat_C)+PEAK_DETECTION.NB_REFACTORY_SAMPLES;
            endIdx = locs(i)-PEAK_DETECTION.NB_REFACTORY_SAMPLES;
            [pks_temp,locs_temp] = max(ecg_int(startIdx:endIdx)); 
            locs_temp = qrs_i(Beat_C)+ PEAK_DETECTION.NB_REFACTORY_SAMPLES + locs_temp -1;% location 
            ...
            if pks_temp > THI2
                ...
                if locs_temp <= length(ecg_n)
                    [y_i_t,x_i_t] = max(ecg_n(locs_temp-PEAK_DETECTION.NB_PEAK_SEARCH_SAMPLES:locs_temp));
                else
                    [y_i_t,x_i_t] = max(ecg_n(locs_temp-PEAK_DETECTION.NB_PEAK_SEARCH_SAMPLES:end));
                end
                
                %********** Band pass Sig Threshold **********%
                if y_i_t > THF2 
                    Beat_C1 = Beat_C1 + 1;
                    qrs_i_raw(Beat_C1) = locs_temp-PEAK_DETECTION.NB_PEAK_SEARCH_SAMPLES + ...
    ...
    if pks(i) >= THI1    
        ...
        if Beat_C >= 3 % After learning phase #2
            if (locs(i)-qrs_i(Beat_C)) <= PEAK_DETECTION.NB_T_WAVE_LOCATION_SAMPLES
                % mean slope of the waveform at that position
                Slope1 = mean(diff(ecg_int(locs(i)-PEAK_DETECTION.NB_SLOPE_SEARCH_SAMPLES:locs(i))));  
                % mean slope of previous R wave
                Slope2 = mean(diff(ecg_int(qrs_i(Beat_C)-PEAK_DETECTION.NB_SLOPE_SEARCH_SAMPLES:qrs_i(Beat_C)))); 
...
noiseIdx = find(qrs_amp_raw <= PEAK_DETECTION.AMP_LOWER_THRESHOLD);
