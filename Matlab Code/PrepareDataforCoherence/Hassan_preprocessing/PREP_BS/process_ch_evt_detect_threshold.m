...
function OutputFiles = Run(sProcess, sInputs) %#ok<DEFNU>   
    ...
    for iFile = 1:length(sInputs)
        ...
        for i = 1:length(detectedEvt)
            ...
            sEvent.times    = [sEvent.times, detectedEvt{i}];
            sEvent.epochs   = [sEvent.epochs, ones(1, size(detectedEvt{i},2))];
            sEvent.channels = [sEvent.channels, repmat({{chanName(:)'}},1,size(detectedEvt{i},2))];
            % Add channel name as event note
            sEvent.notes    =  [sEvent.notes, repmat({{chanName(:)'}},1,size(detectedEvt{i},2))];%repmat({chanName},1, size(sEvent.times, 2)); %cell(1, size(sEvent.times, 2));
