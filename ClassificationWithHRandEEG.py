...
wandb.init(project = "classificationwithhrandeeg")
...
def DataPreparation(BPs, EEGLabels, HRs, Labels):
    ...
    for item in range(len(Labels)):
        ...
        if len(HRs[item]) == len(BPs[item]):
            ...
            locHRs = np.array(newHRs[item][:len(BPs[item])])
            locHRs = locHRs.reshape(-1, 1)
            ConcatenateData = np.concatenate((BPs[item], locHRs), axis=1, dtype = np.double)
        else:
            locBPs = np.array(BPs[item][:len(newHRs[item])])
            locnewHRs = newHRs[item].reshape(-1, 1)
            ConcatenateData = np.concatenate((locBPs, locnewHRs), axis=1, dtype = np.double)
...
kfOuter = KFold(n_splits= 10, shuffle=True, random_state=42)  # Use random_state for reproducibility
AllHyperparameters = {}
AllResults = []
testsets = -1
for trainval_index, test_index in kfOuter.split(full_dataset):
    testsets += 1
    # Create data samplers for training and validation subsets
