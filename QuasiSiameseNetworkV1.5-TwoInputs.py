...
def DataPreparation(BPs, EEGLabels, HRs, Labels):
    ...
    newHRs = []
                                                                            ######### When using only HR and SWIs ############ previous version
    '''for item in range(len(Labels)):
        # Downsampling the longer signal to match the length of the shorter signal
    ...
    ##### Balancing the number of classes
    ...
    dataset = TensorDataset(torch.stack(All_HR_segments_list), torch.stack(All_BP_segments_list), torch.stack(All_labels_list2))
    return dataset
...
class QuasiSiamese(nn.Module):
    def __init__(self, input1_size, input2_size, hidden_size1, num_lstm_layer, num_classes, D, kernel_size, dropout):
        ...
        ####### Fully connected layer
        self.fc = nn.Sequential(
            nn.Linear(2*self.D*self.hidden_size1, 16), ##### change the first number based on the number of input values.
            nn.ReLU(),
...
def HyperparameterTuning(lr, batch_size, num_lstm_layer, kernel_size, dropout, trainval_index, trainval_subset_loader):
    ...
    intargets = np.array([train_val_dataset[item][2] for item in range(len(train_val_dataset))])
    ...
    for train_index, val_index in kf.split(np.zeros(len(intargets)), intargets):
        ...
        model = QuasiSiamese(input_size1, input_size2, hidden_size1, num_lstm_layer, num_classes, D, kernel_size, dropout).to(device)
        model = model.double()
        ...
        for epoch in range(num_epochs):
            ...
            train_total = 0
            model.train()
            for HRsegments, BPsegments, labels in train_subset_loader:
                optimizer.zero_grad()
        ...
        val_loss = 0
        with torch.no_grad():
            for HRsegments, BPsegments, labels in val_subset_loader:
                model.eval()
                labels = labels.to(device)
                HRsegments = HRsegments.to(device)
                BPsegments = BPsegments.to(device)
...
targets = np.array([full_dataset[item][2] for item in range(len(full_dataset))]) # change the number 3 based on the number of the inputs
...
for trainval_index, test_index in kfOuter.split(np.zeros(len(targets)), targets):
    ...
    model2 = QuasiSiamese(input_size1, input_size2, hidden_size1, num_lstm_layer, num_classes, D, kernel_size, dropout).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        train_total = 0
        model2.train()
        for HRsegments, BPsegments, labels in trainval_subset_loader:
            optimizer.zero_grad()
            labels = labels.to(device)
            HRsegments = HRsegments.to(device)
            BPsegments = BPsegments.to(device)
            #Csegments = Csegments.to(device)
            outputs = model2(HRsegments, BPsegments)
            _, predicted = torch.max(outputs, 1)
            train_total += labels.size(0)
            train_correct += (predicted == labels).sum().item()
    ...
    with torch.no_grad():
        ...
        total = 0
        test_loss = 0
        for HRsegments, BPsegments, labels in test_subset_loader:
            model2.eval()
            labels = labels.to(device)
            HRsegments = HRsegments.to(device)
            BPsegments = BPsegments.to(device)
