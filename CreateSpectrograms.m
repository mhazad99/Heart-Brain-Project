...
for i = 1:numel(contents)
    ...
    if item.isdir
        ...
        if length(signal{1,1}.CleanedEEGinfo.CleanedF3) == length(signal{1,2}.ECG.ECGF3)
            EEG = signal{1,1}.CleanedEEGinfo.CleanedF3; gpuEEG = gpuArray(EEG);
            ECG = signal{1,2}.ECG.ECGF3; gpuECG = gpuArray(ECG);
            CalculateSpectrogram(gpuEEG, gpuECG, item.name)
            clear CalculateSpectrogram
        else
            newECGlength = length(signal{1,1}.CleanedEEGinfo.CleanedF3); % cut the end of ECG signal to be aligned with EEG
            EditedECG = signal{1,2}.ECG.ECGF3(1:newECGlength); gpuECG = gpuArray(EditedECG);
            EEG = signal{1,1}.CleanedEEGinfo.CleanedF3; gpuEEG = gpuArray(EEG);
            CalculateSpectrogram(gpuEEG, gpuECG, item.name)
            clear CalculateSpectrogram
        end
...
DFullfilepath = fullfile(targetFolder, 'Spectrograms', 'Arrays', 'ECGF3', 'Depressed', subjectname);
mkdir(DFullfilepath)
for item = 1:loop-1
...
%         save(file1Path, 'scaled_magnitude_S1');
        ...
        save(file2Path, 'scaled_magnitude_S2');
